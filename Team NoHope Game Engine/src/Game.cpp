...
void Game::init()
{
	...
	//_sound = Sound::load(Util::resourcePath + "ruby.oNoHope");
	//_sound2 = Sound::load(Util::resourcePath + "hit.oNoHope");
	...
	//sprite = new NoHope::SpriteEntity(0, 0, 128, 128, texture, _shader); //malli
	//sprite->setPosition(Graphics::screenWidth/2, Graphics::screenHeight/2);
	//sprite->setProjectionMatrix(_projection);

	player = new NoHope::GameObject(Graphics::screenWidth/2, Graphics::screenHeight/2, 64, 64, _player, _shader, &world);
	/*player->setPosition(Graphics::screenWidth/2, Graphics::screenHeight/2);*/
	player->setProjectionMatrix(_projection);

	ground = new NoHope::GameObject(0,-1,10,1, _ground, _shader, &world);
